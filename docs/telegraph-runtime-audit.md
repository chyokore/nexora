...
**Phase 4 resolution notice:** Sigvora's YAML/dispatcher miner ID is `251`; its active on-chain registration ID is `315`. These are separate Telegraph namespaces. Registration 315's immutable IPFS YAML declares `id: 251`, so the dispatcher is correct and the earlier Phase 3C/3D identity blocker is superseded. Historical observations remain below as audit chronology, not current guidance. Nexora gives Sigvora no special routing treatment.
...
# PHASE 3D — AUTHORITATIVE SIGVORA REGISTRATION VERIFICATION

Phase 3D verification timestamp: 2026-09-02T17:58:41+01:00. Scope: official-source research, public explorer inspection, Base Sepolia `eth_call`/log reads, free dispatcher discovery, and public-repository inspection only. Paid inference calls: **0**; payment signatures: **0**; settlements: **0**; blockchain write transactions: **0**; wallet operations: **0**.

## P3D.1 Current chain, deployment, and interface

**VERIFIED FROM OFFICIAL SOURCE:** Telegraph's current validator setup identifies Base Sepolia, chain ID `84532`, and Diamond `0xac683bFa8F1C892E23e8300d14c20678C6FC0CA3`. The current Telegraph API documentation describes that Diamond's `MinerRegistryFacet` and the event-listener-to-database-to-dispatcher hot-load path. Sources: [validator setup](https://github.com/telegraphprotocol/telegraph-docs/blob/main/validators/validator-setup.md), [miner registry](https://github.com/telegraphprotocol/telegraph-api-docs/blob/main/docs/on-chain/miner-registry.md), and [on-chain overview](https://github.com/telegraphprotocol/telegraph-api-docs/blob/main/docs/on-chain/overview.md).

**VERIFIED FROM PUBLIC EXPLORER:** [Base Sepolia Blockscout](https://base-sepolia.blockscout.com/address/0xac683bFa8F1C892E23e8300d14c20678C6FC0CA3) identifies that address as a verified EIP-2535 `Diamond`, deployed in transaction [`0x7fbe850e0fcbf812d43544ef2c93cd48a164f34f708c75371c638d04c5848d7b`](https://base-sepolia.blockscout.com/tx/0x7fbe850e0fcbf812d43544ef2c93cd48a164f34f708c75371c638d04c5848d7b) at block 43,839,253. A live Diamond-loupe call mapped `getMiner(uint256)` to current facet `0x6A7bf84aD287cca1824c94c1BBD28f390ec40fB8`.

The exact relevant read calls found in the current official documentation and verified MinerRegistryFacet reference ABI are `getMiner(uint256)`, `minerCount()`, `getDeregisteredIdCount()`, `getDeregisteredIdAtIndex(uint256)`, and `getCanonicalIntents()`. The relevant writes, inspected only as interface definitions and never called, are `registerMiner(...)` and `deregisterMiner(uint256)`. Relevant events are `MinerRegistered(uint256 indexed registrationId,address indexed miner,string yamlUrl,bytes32 yamlHash,address feeAddress,uint256 minPriceUsdc,string[] supportedIntents)` and `MinerDeregistered(uint256 indexed registrationId,address indexed miner)`. **VERIFIED FROM OFFICIAL SOURCE AND PUBLIC EXPLORER.**

## P3D.2 Authoritative reads for identifiers 251 and 315

At Base Sepolia blocks 46,299,736–46,299,830, `minerCount()` returned **152**. `getDeregisteredIdCount()` returned **26**, and the complete indexed deregistration list was `1, 4, 5, 6, 7, 9, 10, 11, 8, 15, 16, 13, 2, 27, 32, 33, 34, 31, 29, 35, 43, 44, 55, 60, 109, 150`. `getMiner(251)` and `getMiner(315)` both returned the empty default record: zero miner/fee addresses and hashes, empty YAML URL and intent list, zero price, and `active=false`. Bounded event-log queries from the Diamond deployment block found no `MinerRegistered` event for either 251 or 315 and no `MinerDeregistered` event for 251. The count and empty records independently establish that neither number is a contract registration ID on this deployment. **VERIFIED FROM BASE SEPOLIA ON-CHAIN DATA.**

Every contract record 1–152 was also inspected read-only. No miner or fee address matched dispatcher-visible Sigvora fee address `0x0530F3e4E94e9a183D785e094790F5660FB50F10`, and no YAML URL contained `sigvora` or `chyokore`. **VERIFIED FROM BASE SEPOLIA ON-CHAIN DATA.** Therefore this Diamond provides no on-chain transaction, block, operator, URI, intent, or removal evidence attributable to Sigvora #251 or #315.

The likely explanation is an ID-namespace or deployment mismatch: dispatcher/YAML `id` values are not bounded by this Diamond's registration counter, while on-chain events name a distinct `registrationId`. That explanation is **INFERRED**, not proven by the published documentation. It would also be possible that a newer registry deployment is used by the dispatcher but is absent from the inspected current official configuration; no official source identifying such a deployment was found.

## P3D.3 Published Sigvora and dispatcher evidence

**VERIFIED FROM REPOSITORY:** Sigvora's public repository README now claims active Telegraph miner `#315`, but its only published miner document, `telegraph/sigvora-miner.yaml`, still declares `id: 251`. That YAML declares slug `sigvora`, name `Sigvora Fraud Intelligence`, `FRAUD_DETECTION`, `POST /analyze`, base URL `https://sigvora-staging.onrender.com`, and `0.01` USDC minimum price, but no input or output JSON schemas. No public YAML declaring ID 315 was found. The README claim is repository evidence, not independent on-chain evidence, and conflicts with the repository's YAML.

**VERIFIED FROM FREE LIVE REGISTRY:** At 2026-09-02T16:49:52+01:00, the dispatcher raw-YAML endpoint still returned the same `id: 251` Sigvora YAML; the integrations endpoint still exposed 251 as active and omitted 315. No miner endpoint was called.

The required chain-versus-dispatcher classification is **CHAIN STATE INCONCLUSIVE**. The official Diamond cannot corroborate either dispatcher/YAML ID as an on-chain registration ID, so it would be unsound to label the dispatcher stale solely by comparing unlike or unresolved namespaces. The project owner's statement that 251 is historical/deregistered and 315 current remains **PROJECT-OWNER CONFIRMED**, but is not upgraded to on-chain verification.

Miner 315 eligibility is **UNVERIFIED**. No authoritative current 315 record or YAML was discovered, so registered/live status, schema compatibility, price, rank, and score cannot be evaluated. The published 251 YAML is not substituted for 315; even if it were, its absent input/output schemas would fail Nexora's unchanged non-null-schema rule.

## P3D.4 Reload path, escalation, and safety decision

Official architecture states that `MinerRegistered` events trigger a listener which fetches and hash-verifies YAML, persists it in `miner_registry`, and hot-reloads the dispatcher; it also names startup rehydration. **VERIFIED FROM OFFICIAL SOURCE.** No public user-callable resync, cache-flush, or registry-reindex procedure was found. Any reconciliation therefore requires the Telegraph operator/team to identify the authoritative deployment and ID mapping, then inspect/replay or repair its listener/database state if appropriate. This conclusion is **INFERRED** from the documented architecture.

Prepared escalation text (not sent):

> At 2026-09-02T17:58:41+01:00, `GET http://13.237.89.59:7044/miner-dispatcher/integrations` and `/yamls` exposed Sigvora ID 251 as active and omitted owner-reported current ID 315. Current official docs point Base Sepolia (84532) to Diamond `0xac683bFa8F1C892E23e8300d14c20678C6FC0CA3`. Read-only calls at block 46,299,830 returned `minerCount=152`, empty `getMiner(251)`/`getMiner(315)` records, and no indexed registration logs for either ID since deployment block 43,839,253; all records 1–152 contained no Sigvora URL/address match. Sigvora's public README says 315, while its miner YAML still declares 251. Could the Telegraph team confirm the authoritative registry deployment and whether dispatcher YAML IDs map separately from contract registration IDs, then inspect listener/indexer state? This affects reproducible live discovery and MCP routing. No paid inference, signing, wallet, or chain-write action was performed.

Historical Phase 3D recommendation (superseded by the Phase 4 resolution notice above): **STOP all signer integration and paid FRAUD_DETECTION tests until Telegraph confirms the authoritative registry/ID mapping and #315 becomes independently discoverable with current metadata.** Proceeding with Anchor merely because it is dispatcher-visible would leave the discovery-integrity issue unresolved; chain-aware eligibility is not selected because current official architecture does not document a consumer-side join from dispatcher YAML IDs to Diamond registration IDs. No routing logic or harness code was changed.

# PHASE 4 — SECURE TELEGRAPH X402 EXECUTION PREPARATION

Phase 4 free-registry timestamp: 2026-09-02T18:24:27.732Z. The dispatcher returned 129 registrations. Under the unchanged ownership-blind rule, eligible counts and winners were: FRAUD_DETECTION 6, Anchor 49; URL_SCAN 7, NetWire 7334; ONCHAIN_TX_LOOKUP 11, Veyctum 9005; FACT_CHECK 3, Qarinah 717190; NEWS_SEARCH 3, Tavily 202. **VERIFIED FROM FREE LIVE REGISTRY.** Sigvora 251 was active at rank 8 but ineligible because both schemas were null, exactly as for any other schema-less miner.

The isolated smoke harness now uses the official Telegraph MCP's EVM stack (`@x402/fetch`, `@x402/evm`, and `viem`) behind a Nexora adapter. Default execution remains free discovery only. Paid behavior requires `--execute-paid`, a recognized logical test ID, an environment-only `TELEGRAPH_EVM_PRIVATE_KEY`, exact `EVM_NETWORK=eip155:84532`, and an explicitly approved asset. The adapter obtains an unsigned 402 first, rejects redirects/target substitution, validates scheme/network/asset/amount/payee/expiry/miner-intent binding, authorizes the logical ID in the three-call ledger, and only then derives the signer and performs the normal x402 negotiation. No asset is inferred from a generic example.

The fixed limits remain three logical purchases, 10,000 micro-USDC each, and 30,000 cumulative. Capture excludes credentials and raw signatures and records the negotiation status sequence, selected miner, contract family, authorized amount, sanitized response metadata, domain evidence, conformance, errors, and settlement state. `FraudEvidence`, `UrlSafetyEvidence`, `OnchainTransactionEvidence`, and `FactCheckEvidence` preserve domain facts and missing values; confidence is never fabricated.

Validation: 28 tests passed, strict TypeScript compilation passed, and the free default dry-run passed. Paid calls, signatures, settlements, blockchain writes, and wallet operations were all zero. A future controlled run still requires a dedicated minimally funded burner, independently approved live challenge asset, explicit authorization, and execution of exactly the three planned logical IDs.

Phase 5B.1 separates miner-level intent/schema eligibility from endpoint-level intent compatibility. The registry provides endpoint `path`, `method`, and optional `description`, but no dedicated endpoint-intent field. Selection now prefers a single canonical intent declaration in the endpoint description, otherwise accepts only narrow intent-specific path/description evidence, rejects ambiguity, evaluates every endpoint independently, and applies a stable endpoint tie-break without changing miner rank/score priority. This generic rule contains no miner, owner, or project special cases.
