...
**Phase 4 identity resolution:** Sigvora's YAML/dispatcher miner ID is 251 and its active on-chain registration ID is 315. Registration 315's immutable IPFS YAML declares `id: 251`; the namespaces are distinct and the dispatcher row below is current as a routing identity. Sigvora remains ineligible under the neutral non-null-schema rule and receives no special treatment.
